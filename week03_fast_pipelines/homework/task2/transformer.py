...
class TransformerLetsGoooooooooooo(nn.Module):
    def __init__(self,
        ...
        self.get_emb = nn.Embedding(vocab_size, d_model)
        self.pos_encoder = PositionalEncoding(d_model=d_model, dropout=dropout)
        decoder_layer = nn.TransformerDecoderLayer(d_model=d_model, nhead=nhead)
        self.decoder = nn.TransformerDecoder(decoder_layer=decoder_layer, num_layers=nlayers)

    def forward(self, tgt: Tensor, src: Tensor) -> Tensor:
        tgt = self.get_emb(tgt)
        src = self.get_emb(src)
